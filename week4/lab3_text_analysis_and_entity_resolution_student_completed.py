...
def simpleTokenize(string):
    ...
    """
    tmp = re.split(split_regex,string.lower())
    
    def f(x): return x != ''
    
    return filter(f, tmp) #re.split(split_regex,string.lower())
...
def tokenize(string):
    ...
    tmp = re.split(split_regex,string.lower())
    def f(x): return x != ''
    tmp2 = filter(f, tmp) #re.split(split_regex,string.lower())
    
    def f1(x): return x not in stopwords
    
    return filter(f1 ,tmp2)
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda x: ( x[0] , tokenize(x[1])))#<FILL IN> <class 'pyspark.rdd.PipelinedRDD'>
googleRecToToken = googleSmall.map(lambda x: ( x[0] , tokenize(x[1])))#<FILL IN>
...
# print amazonRecToToken.map(lambda x: len(x[1])).take(12) # debug code
...
def idfs(corpus):
    ...
    """
    N = corpus.count() # find number of documents in corpus
    uniqueTokens = corpus.flatMap(lambda x: list(set(x[1]))) #
    # print uniqueTokens.take(5)
