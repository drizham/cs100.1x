...
# length(s[1]) gives the total number of host requests
# then len(list(set(s[1]))) gives the unique number of hosts on the day
avgDailyReqPerHost = (sortedByDay.map(lambda s: (s[0], len(s[1]) / len(list(set(s[1])))) )).cache()
...
#badEndpointsCountPairTuple = badRecords.map(lambda x: x.endpoint).map(lambda x: (x, 1))
...
errDateSum = errDateCountPairTuple.reduceByKey(lambda a, b: a + b ) # reduceByKey(lambda a, b: a + b)
...
hourCountPairTuple = badRecords.filter(lambda x: x.response_code == 404).map(lambda x: (x.date_time.hour, 1))
